...
class Controller:
    ...
    def handleAnalizza(self, e):
        ...
        try:
            ...
            self._view._txt_result.controls.append(ft.Text(f"Il valore inserito nel campo nMin non è un intero"))
            self._view.update_page()

        self._model.buildGraph(nMin)

        self._view._txt_result.controls.append(ft.Text(f"Grafo correttamento creato"))
        self._view._txt_result.controls.append(ft.Text(f"Num nodi: {self._model.getNumNodi()}"))
        self._view._txt_result.controls.append(ft.Text(f"Num arcgi: {self._model.getNumArchi()}"))
        self._view.update_page()
    ...
    def handleConnessi(self, e):
        if self._choiceAeroportoP is None:
            self._view._txt_result.controls.append(ft.Text(f"Selezionare un aereoporto di partenza"))
            return
    ...
    def handleTestConnessione(self, e):
        ...
        if not self._model.esistePercorso(v0, v1):
            self._view._txt_result.controls.append(ft.Text(f"Non esite un percorso fra {v0} e {v1}."))
            return
        else:
        ...
        self._view._txt_result.controls.append(ft.Text(f"Il cammino con minor numero di arvhi tra {v0} e {v1} è:"))

        for p in path:
            self._view._txt_result.controls.append(ft.Text(f"{p}"))

        self._view._txtInNumC.disabled = False
